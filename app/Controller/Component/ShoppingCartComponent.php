 ...
 */
 ...
 class ShoppingCartComponent extends Object {
	...
	function resetCart($full = true) {
		$data = null;
		if ($data = $this->controller->Session->read('Order') && !$full) {
			unset($data['LineItem'], $data['Billing'], $data['Shipping'], $data['Total']);
			return $this->controller->Session->write('Order', $data);
		} else {
			return $this->controller->Session->delete('Order');
		}
	}
	...
	function addItem($data, $quantity = 1, $attribs = array()) {
		...
		if ($this->useTable) { 
			if (is_array($data)) {
				...
				if ($this->controller->Session->check('Order.LineItem.' . $data)) {
						$lineItem = $this->controller->Session->read('Order.LineItem.' . $data);
				} else {
		...
		$lineItems = array();
		if ($this->controller->Session->check('Order.LineItem')) {
			$lineItems = $this->controller->Session->read('Order.LineItem');
		}
	...
	function _addItemNew($lineItem, $attribs = array(), $quantity, $lineItems) {
		...
		$returnValue = $this->controller->Session->write("Order.LineItem", $lineItems);
		$this->calcTotal();
...
/*		if (!empty($product)) {
			$returnValue = $this->controller->Session->write("Order.LineItem.$id.Product", $product);
		}*/
		...
		if ($quantity <= 0) {
			...
			$selection = $this->controller->Session->read("Order.LineItem.$id.Selection");

			$selection[$selectionCount]['subtotal'] = $quantity * $selection[$selectionCount]['price']; 
			$selection[$selectionCount]['quantity'] = $quantity;

			$returnValue = $this->controller->Session->write("Order.LineItem.$id.Selection", $selection);
			$this->calcTotal();
		}
		return $returnValue;
	}
	...
	function removeItem($id, $selection=0) {
		$returnValue = $this->controller->Session->delete("Order.LineItem.$id.Selection.$selection");

		$selectionArray = $this->controller->Session->read("Order.LineItem.$id.Selection");

		if (empty($selectionArray)) {
			$this->controller->Session->delete("Order.LineItem.$id");
		}
	...
	function getOrderDetails() {
		if ($this->controller->Session->check('Order')) {
			return $this->controller->Session->read('Order');
		} else {
			return false;
		}
	}
	...
	function setBillInfo($data) {
		return $this->controller->Session->write('Order.Billing', $data);
	}
